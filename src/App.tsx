...
export default function App() {
  ...
  const obtenerEquiposFirebase = async () => {
    try {
      ...
      setListaEquipos(equiposCargados);
  ...
  const obtenerPartidosHistorial = async () => {
    try {
      ...
      partidosCargados.sort((a, b) => b.fecha.localeCompare(a.fecha));
      setListaPartidosViejos(partidosCargados);
    } catch (error) {
  ...
  useEffect(() => {
    obtenerEquiposFirebase().then((equipos) => {
      if (equipos && equipos.length > 0) {
        ...
        setJugadorasDelEquipo(equipos[0].jugadoras || []);
      }
    });
  ...
  const crearNuevoEquipo = async (e: React.FormEvent) => {
    ...
    try {
      await setDoc(doc(db, 'equipos', idSugerido), {
        nombre: nuevoNombreEquipo,
        jugadoras: [],
      });
      setNuevoNombreEquipo('');
      await obtenerEquiposFirebase();
      manejarCambioEquipo(idSugerido);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const agregarJugadorasAlEquipo = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoc(doc(db, 'equipos', equipoSeleccionado), {
  ...
  const comenzarPartidoEnBaseDeDatos = async (e: React.FormEvent) => {
    ...
    await setDoc(doc(db, 'partidos', nuevoId), {
      id_partido: nuevoId,
  ...
  const eliminarPartidoHistorial = async (idPart: string) => {
    ...
    if (!seguro) return;
    try {
      await deleteDoc(doc(db, 'partidos', idPart));
      setPartidoHistorialSeleccionado(null);
      await obtenerPartidosHistorial();
    } catch (e) {
      console.error(e);
    }
  };
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: '24px', marginTop: '4px' }}>
          {estadisticas[cuartoActual][campo]}
        </div>
      </button>
    );
  };
              ...
              {!partidoHistorialSeleccionado ? (
                ...
                >
                  {listaPartidosViejos.length === 0 ? (
                    ...
                    >
                      No hay partidos registrados en Firebase
                    </div>
                  ) : (
                    listaPartidosViejos.map((p) => (
                      <button
                        key={p.id}
            ...
            >
              ...
              >
                <label style={{ fontSize: '13px', color: '#94a3b8' }}>
                  Sumar jugadoras a la categoría seleccionada (
                  <b>
              ...
              {equipoSeleccionado && jugadorasDelEquipo.length > 0 && (
                <div style={{ marginTop: '4px' }}>
                  ...
                  >
                    Lista Actual del Plantel (Click en ❌ para dar de baja):
                  </label>
                  <div
                    style={{
                      display: 'flex',
            ...
            >
              ...
              >
                {jugadorasDelEquipo.length === 0 ? (
                  ...
                  >
                    Este plantel no tiene jugadoras. Sumalas arriba en el Panel.
                  </div>
                ) : (
        ...
        <div>
          ...
          {vista === 'telefono' && (
            ...
            >
              ...
              >
                💡 Tip: Mantené presionado cualquier botón para restar 1
              </p>
            </div>
          )}
          ...
          {vista === 'computadora' && (
            <div style={{ maxWidth: '950px', margin: '0 auto' }}>
              ...
              >
                <thead>
                  <tr>
                    <th style={estiloCeldaTh}>Métrica / Evento</th>
                    <th style={estiloCeldaTh}>1° Cuarto</th>
                    <th style={estiloCeldaTh}>2° Cuarto</th>
                    <th style={estiloCeldaTh}>3° Cuarto</th>
                    <th style={estiloCeldaTh}>4° Cuarto</th>
                    <th
                      style={{ ...estiloCeldaTh, backgroundColor: '#2563eb' }}
                    >
                      TOTAL
                    </th>
                  </tr>
